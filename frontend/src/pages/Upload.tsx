...
export default function Upload() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center text-white px-4 overflow-hidden">
      ...
      <motion.p className="text-gray-300 mb-12 text-center text-lg tracking-wide" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }}>
      ...
      <motion.div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl shadow-xl p-10 w-full max-w-lg text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-400 rounded-xl py-10 cursor-pointer hover:border-purple-400 transition">
          ...
          <p className="text-gray-300 mb-4 text-sm">{fileName ? fileName : "Drag & drop a file or click to upload"}</p>
          <label
            htmlFor="fileInput"
      ...
      {/* === Results === */}
      {result && (
        <motion.div className="mt-12 backdrop-blur-lg bg-white/10 border border-white/20 rounded-2xl shadow-lg p-6 w-full max-w-lg text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <h2 className="text-2xl font-bold text-purple-300 mb-4">Analysis Result</h2>
          ...
          <p className="text-green-400 font-semibold mt-2">Status: {result.status}</p>
        </motion.div>
      )}

      {/* === Footer === */}
      <p className="mt-20 text-gray-400 text-sm">© 2025 TrueView — Built for the Trust & Integrity Hackathon</p>
    </div>
  );
}
